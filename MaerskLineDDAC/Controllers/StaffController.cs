    ...
    public class StaffController : Controller
    {
        ...
        public ActionResult Create(ExpandedUserDTO paramExpandedUserDTO)
        {
            try
            {
                ...
                var objNewStaffUser = new ApplicationUser { UserName = UserName, Email = Email };
                var StaffUserCreateResult = UserManager.Create(objNewStaffUser, Password);

                if (StaffUserCreateResult.Succeeded == true)
                {
                    ...
                    {
                        // Put user in role
                        UserManager.AddToRole(objNewStaffUser.Id, strNewRole);
                    }
        ...
        // DELETE: /Staff/DeleteRole?UserName="TestUser&RoleName=Staff
        [Authorize(Roles = "Staff")]
        ...
        public ActionResult DeleteRole(string UserName, string RoleName)
        {
            try
            {
                ...
                if (UserName.ToLower() ==
                    this.User.Identity.Name.ToLower() && RoleName == "Staff")
                {
                    ModelState.AddModelError(string.Empty,
                        "Error: Cannot delete Staff Role for the current user");
                }
        ...
        public ActionResult AddRole(RoleDTO paramRoleDTO)
        {
            try
            {
                ...
                return Redirect("~/Staff/ViewAllRoles");
            }
            catch (Exception ex)
            {
                ModelState.AddModelError(string.Empty, "Error: " + ex);
        ...
        // DELETE: /Staff/DeleteUserRole?RoleName=TestRole
        [Authorize(Roles = "Staff")]
